...
const WeatherBoxView = ({ weatherData, getWeatherImage, getTimeString, getDateString, getThemeColor }: IWeatherBoxProps): JSX.Element => (
    <div className='weather-data-container' style={{ backgroundColor: getThemeColor() }}>
        ...
        <div className='weather-inner-container'>
            <p style={{ fontWeight: 'bold', fontSize: 27 }}> {parseInt(weatherData.main.temp)}° </p>
            <p style={{ fontSize: 20 }}> {getDateString()} </p>
            <div>
                <p style={{ fontSize: 15 }}> Min: {parseInt(weatherData.main.temp_min)}° </p>
                <p style={{ fontSize: 15 }}> Max: {parseInt(weatherData.main.temp_max)}° </p>
            </div>
        </div>
    </div>
)
